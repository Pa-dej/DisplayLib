-- Lua API Demo Script (Fixed Version)
-- Демонстрирует все возможности DisplayLib Lua API без проблемных конструкций
...
function btn_change_text_click()
    ...
    if demo_text then
        ...
        if current_text_index > 5 then
            current_text_index = 1
        end
        
        local texts = {"Изменяемый текст", "Новый текст!", "Lua работает!", "DisplayLib API", "Отлично! 🎉"}
        local new_text = texts[current_text_index]
...
function btn_change_color_click()
    ...
    if demo_text then
        ...
        if current_color_index > 5 then
            current_color_index = 1
        end
        
        local colors = {{80, 80, 120}, {120, 80, 80}, {80, 120, 80}, {120, 120, 80}, {120, 80, 120}}
...
function btn_timer_repeat_click()
    ...
    if repeat_timer_id then
        ...
        -- Запускаем таймер (простая версия)
        local tick_count = 0
        repeat_timer_id = timer["repeat"](10, function()  -- каждые 0.5 секунды
            ...
            if status then
                ...
                -- Простое мигание без модуля
                if tick_count > 10 then tick_count = 1 end
...
function btn_timer_animation_click()
    ...
    timer.times(20, 5, function(i)  -- каждые 1 секунду, 5 раз
        if status then
            ...
            if i == 1 then
                status.bgColor(85, 33, 33)  -- красный
            elseif i == 2 then
                status.bgColor(85, 66, 33)  -- оранжевый
            elseif i == 3 then
                status.bgColor(85, 85, 33)  -- желтый
            elseif i == 4 then
                status.bgColor(33, 85, 33)  -- зеленый
            else
                status.bgColor(33, 33, 85)  -- синий
            end
        end
